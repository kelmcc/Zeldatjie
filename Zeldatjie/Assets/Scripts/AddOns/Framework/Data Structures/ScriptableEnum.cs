    ...
    public abstract class ScriptableEnum : ScriptableObject
    {
        ...
        void EnsureCorrectGUID()
        {
            if (UnityEditor.AssetDatabase.TryGetGUIDAndLocalFileIdentifier(this, out string guid, out long localID))
            {
                if (_guid != guid)
                {
                    Debug.LogError($"[ScriptableEnum] Ensure correct GUID, failed, reassigning. THIS INDICATES THE BUILD WILL BREAK. Ensure all scriptable enum assets are in a Resources folder: {this.name}");
                    _guid = guid;
                    SerializedObject serializedObject = new SerializedObject(this);
